...
public class ShadeView extends View{
    ...
    public ShadeView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //获取自定义属性值
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.ShadeView);
        BitmapDrawable drawable = (BitmapDrawable) ta.getDrawable(R.styleable.ShadeView_icon);
        if (drawable != null){
            iconBitmap = drawable.getBitmap();
        }
        iconBackgroundColor = ta.getColor(R.styleable.ShadeView_color, DEFAULT_ICON_BACKGROUND_COLOR);
        text = ta.getString(R.styleable.ShadeView_text);
        int textSize = (int) ta.getDimension(R.styleable.ShadeView_text_size,
    ...
    /**
     * 在mBitmap上绘制以iconBackgroundColor颜色为Dst，
     * DST_IN模式下的图标
     * @param alpha
     */
